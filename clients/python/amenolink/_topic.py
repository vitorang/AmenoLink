...
from ._shared import AmenoException, T, client_setup, _parse_data, _post_json
...
class Topic(Generic[T]):
    def __init__(self, name: str, value_type: type[T] = Any):
        ...
        self._handlers: set[Callable[[TopicMessage[T]], None]] = set()

    def subscribe(self, handler: Callable[[TopicMessage[T]], None]) -> None:
    ...
    def publish(self, value: T | None = None, previous: Message | None = None) -> None:
        ...
        url = f'{client_setup.origin_url}/api/topic/publish'
    ...
    def _dispatch_message(self, message: TopicMessage) -> None:
        ...
        if self.value_type is not Any and message.payload is not None:
            parsed_payload = _parse_data(message.payload, self.value_type)

        message_to_dispatch = TopicMessage(
            id=message.id,
        ...
        for handler_function in list(self._handlers):
            handler_function(message_to_dispatch)
